...
# WRITE YOUR SOLUTION BELOW:
FIRST_OUTPUT_VALUE=$(bitcoin-cli decoderawtransaction "$BASE_TX" | jq -r '.vout[0].value' | awk '{print $1 * 100000000}')
...
# WRITE YOUR SOLUTION BELOW:
UTXO_TXID=$TXID
UTXO_VOUT_INDEX=$(bitcoin-cli decoderawtransaction "$BASE_TX" | jq -r '.vout | to_entries[] | select(.value.value * 100000000 >= 15000000) | .key' | head -n 1)
check_cmd "UTXO vout selection" "UTXO_VOUT_INDEX" "$UTXO_VOUT_INDEX"

UTXO_VALUE=UTXO_VALUE=$(bitcoin-cli decoderawtransaction "$BASE_TX" | jq -r ".vout[$UTXO_VOUT_INDEX].value" | awk '{print $1 * 100000000}')
...
PARENT_TXID=$(bitcoin-cli listtransactions "*" 1 | jq -r '.[0].txid')
...
# WRITE YOUR SOLUTION BELOW:
CHANGE_OUTPUT_INDEX=$(echo "$RAW_PARENT_TX" | jq -r '.vout | to_entries[] | select(.value > 0.0001) | .key')
...
CHILD_SEND_AMOUNT=$(echo "$CHANGE_AMOUNT - ($CHILD_FEE_SATS / 100000000)" | bc)
check_cmd "Child amount calculation" "CHILD_SEND_AMOUNT" "$CHILD_SEND_AMOUNT"

# Convert to BTC
CHILD_SEND_BTC=$(printf "%.8f" "$CHILD_SEND_AMOUNT")
...
SECONDARY_TXID=$(bitcoin-cli listtransactions "*" 1 | jq -r '.[0].txid')
...
# WRITE YOUR SOLUTION BELOW:
TIMELOCK_INPUTS='[{"txid":"'$SECONDARY_TXID'","vout":'$SECONDARY_OUTPUT_INDEX',"sequence":10}]'
...
SECONDARY_OUTPUT_VALUE=$(echo "$RAW_SECONDARY_TX" | jq -r ".vout[$SECONDARY_OUTPUT_INDEX].value")
